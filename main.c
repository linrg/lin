...

{
 ...
 int locX, locY /* €UŽÑŠìžm */, count /* ³sŽÑŒÆ */, x, y /* ŒÈŠsŠìžm */, displace_x=0, displace_y=0 /* Šì²Ÿ¶q */, direction;

 ArrayList steps = new ArrayList(); /* °O¿ýŽÑšB */



 JPopupMenu control_menu = new JPopupMenu(); /* ¥kÁä¿ï³æ¥\¯à */

 JMenuItem[] command = new JMenuItem[4];

 String[] command_str={"®¬ŽÑ", "ŠsÀÉ", "ÅªÀÉ", "­«¶}"};
 ...
 final JDialog dialog = new JDialog(this, "œÐ¿ï€l", true);
 ...
 final static int nil=-1; /* µL€èŠV */

 final static int oblique_1 =0; /* ¥k€WŠV¥ª€U */

 final static int oblique_2 =1; /* ¥ª€WŠV¥k€U */

 final static int horizontal =2; /* ŸîŠV */

 final static int vertical=3; /* ªœŠV */
...

 {
  ...
  choice[0] = new JRadioButton("¶Â€l", new ImageIcon(black), true);

  choice[1] = new JRadioButton("¥Õ€l", new ImageIcon(white));
  ...
  JButton select = new JButton("œT©w");
...

  {
...

   {
...

    {
     ...
     if(x-step*displace_x<0 || x-step*displace_x>=length || y-step*displace_y<0 || y-step*displace_y>=length || !gobang_SpaceAI(x+displace_x, y+displace_y) || (grid[x-step*displace_x][y-step*displace_y]!=0 && step<3) )  /* ¹ïÂIª¬ªp */
 ...
 private boolean gobang_SpaceAI(int x, int y) /* ­pºâ³ÑŸlªÅ¥ÕŠìžm */

 {
  ...
  if(space+count>=5) /* ³ÑŸlªÅ¥Õ³¡€ÀŠ³³sœu€§¥i¯à */
...

 {
  ...
  if(space+step>=5)
   ...
   setMark(x-(step+1)*displace_x, y-(step+1)*displace_y); /* §ï€U¹ïÂI */
...

 {
...

  {
...

   {
    ...
    JOptionPane.showMessageDialog(null, "šSŠ³ŠsÀÉ");
